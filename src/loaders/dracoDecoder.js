...
export async function initDracoDecoder(path) {
	if (loader.pending) {
    ...
    */
	...
	const wrapperText = await (await wrapperLoader).text();
	const wasmArrayBuffer = await (await wasmLoader).arrayBuffer();
	const workerSource = getWorkerSourceURL(wrapperText);

	const worker = new Worker(workerSource);
	worker._callbacks = {};
	worker.postMessage({ type: "init", decoderConfig: { wasmBinary: wasmArrayBuffer } });
...
function getWorkerSourceURL(jsText) {

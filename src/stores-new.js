...
export const programs = derived([scene, meshes, materials], ([$scene, $meshes, $materials]) => {
	...
	const specialMeshes = new Set($meshes.filter((node) => node.instances > 1 || node.animations?.some(a => a.type === "vertex")));
	...
	return programs.map(p => 
		({
			...p,
			createProgram: createProgram(p.material),
...
const webglapp = derived(
	...
	([$renderer, $programs]) => {
		...
		const lights = $scene.filter(isLight);
			...
			renderPipeline.push(
				...$programs.reduce((acc, program) => {
					...
					return [
						...
						program.createShaders(appContext, program.meshes),
						...
						...program.meshes.reduce((acc, mesh) => [
							...acc,

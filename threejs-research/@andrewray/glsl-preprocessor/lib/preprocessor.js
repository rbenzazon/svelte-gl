...
const spliceStr = (str, idx, rem, s) => str.slice(0, idx) + s + str.slice(idx + rem);
...
Processor.prototype.addDefines = Preprocessor.prototype.addDefines = function (line, withConst, withMacros) {
	// Local variables
	let i1 = -1;
	let i2;
	let d, r;
	...
	for (let i in this.defines) {
		...
		for (;;) {
			...
			if (isIdentChar(line.charAt(i1 - 1)) || isIdentChar(line.charAt(i1 + i2))) continue;
...
// Create a constant
Processor.prototype.createConstant = Preprocessor.prototype.createConstant = function (name, value, addDefines) {
...
Processor.prototype._compConst = Preprocessor.prototype._compConst = function (name, value) {
	this.createConstant("__" + name + "__", value, false);
};

// Add a constant in a line
Processor.prototype.addConstant = Preprocessor.prototype.addConstant = function (line, i, constant) {
...
Processor.prototype.createMacro = Preprocessor.prototype.createMacro = function (name, text) {
	...
	for (let i = 0, l = args.length, p; i < l; i++) {
		...
		for (;;) {
			...
			if (isIdentChar(text.charAt(i1 - 1)) || isIdentChar(text.charAt(i1 + i2))) continue;
...
Processor.prototype.addMacro = Preprocessor.prototype.addMacro = function (line, i, macro) {
	let m = 0;
	...
	for (s = 0, l = macro.pos.length; s < l; s++) str += args[macro.pos[s]] + macro.content[s + 1];
...
Processor.prototype.conditionNext = function (end) {
	...
	while (this.currentLine < this.linesCount) {
		...
		else if (!end && n == 1 && elseCmd.indexOf(s) != -1) return this.callCondition(line);
		else if (s == "endif") {
			n--;
			if (n == 0) return;
		}
...
Processor.prototype.callCondition = function (text) {
	...
	const split = text.substr(1).trimLeft().split(" "),
		name = split[0];
...
createDirective("if", function (expr) {
	...
	let filledExpr = expr.replace(/defined\s*\(\s*([\s\S]+?)\s*\)/g, function (match, p1) {

...
function clearUnusedCache(next) {
	const { programMap, vaoMap } = appContext$1;
		...
		*/
		...
		pipeline.push(
			...sortedPrograms.reduce((acc, program) => {
				appContext$1.existingProgram = false;
				...
				return [
					...acc,
					...(appContext$1.programMap.has(program)
					...
					...(program.setupCamera
						? [program.setupCamera]
						: [...(updateMap.has(camera) || !appContext$1.existingProgram ? [setupCamera($camera)] : [])]),
					...(program.setFrameBuffer ? [program.setFrameBuffer] : []),
					...program.meshes.reduce(
						(acc, mesh) => [
							...acc,
							...(appContext$1.vaoMap.has(program) && appContext$1.vaoMap.get(program).has(mesh)
								? [
...
function initRenderer() {
	const canvasRect = appContext$1.canvas.getBoundingClientRect();
	appContext$1.canvas.width = canvasRect.width;
	appContext$1.canvas.height = canvasRect.height;
	/** @type {WebGL2RenderingContext} */
	const gl = appContext$1.canvas.getContext("webgl2");
	appContext$1.gl = gl;
...
function createProgram(programStore) {
	return function createProgram() {
		...
		appContext$1.programMap.set(programStore, program);
		appContext$1.vaoMap.set(programStore, new Map());
		appContext$1.program = program;
	};
}
...
function bindDefaultFramebuffer() {
	const { gl, backgroundColor } = appContext$1;
	gl.bindFramebuffer(gl.FRAMEBUFFER, null);
	gl.viewport(0, 0, gl.canvas.width, gl.canvas.height);
	appContext$1.frameBufferWidth = gl.canvas.width;
	appContext$1.frameBufferHeight = gl.canvas.height;
	...
	return function createShaders() {
		...
		const fragmentShaderSource = templateLiteralRenderer(defaultFragment, {
			...
			declarations: [
				...
				...(appContext$1.toneMappings?.length > 0
					? [...appContext$1.toneMappings.map((tm) => tm.shader({ declaration: true, exposure: tm.exposure }))]
					: []),
			...
			toneMapping: [
				...(appContext$1.toneMappings?.length > 0
					? [...appContext$1.toneMappings.map((tm) => tm.shader({ color: true }))]
					: []),
			].join("\n"),
...
function setupAmbientLight(programOverride, ambientLightColorOverride) {
	const { gl, program, ambientLightColor } = appContext$1;
...
function setupCamera(camera) {
	return function createCamera() {
		const { gl, program, canvas } = appContext$1;
...
function setupTransformMatrix(programStore, mesh, transformMatrix, numInstances) {
	if (numInstances == null) {
		...
		return function setupTransformMatrix() {
			...
			const { gl, program, vaoMap } = appContext$1;

			//TODO, clean that it's useless since we overwrite it anyway and storing this way is not good
			let transformMatricesWindows;
			if (appContext$1.transformMatricesWindows == null) {
				transformMatricesWindows = [];
			} else {
				transformMatricesWindows = appContext$1.transformMatricesWindows;
			}
...
function updateTransformMatrix(programStore, worldMatrix) {
	const { gl, programMap } = appContext$1;
...
function updateInstanceTransformMatrix(programStore, mesh, newMatrix, instanceIndex) {
	const { gl, vaoMap, matrixBuffer } = appContext$1;
...
function setupNormalMatrix(programStore, mesh, numInstances) {
	if (numInstances == null) {
		...
		return function setupNormalMatrix() {
			const { gl, program, vaoMap, transformMatricesWindows } = appContext$1;
			...
			appContext$1.normalMatrixBuffer = normalMatrixBuffer;
...
function updateInstanceNormalMatrix(programStore, mesh, normalMatrix, instanceIndex) {
	const { gl, vaoMap, normalMatrixBuffer } = appContext$1;
	gl.bindVertexArray(vaoMap.get(programStore).get(mesh));
	gl.bindBuffer(gl.ARRAY_BUFFER, normalMatrixBuffer);
	const bytesPerMatrix = 4 * 16;
...
export { multiply as $, set_store_value as A, skyblue as B, createLightStore as C, createPointLight as D, create3DObject as E, createOrbitControls as F, binding_callbacks as G, createMaterialStore as H, cross as I, subtract as J, normalize as K, drawModes as L, Menu as M, rotateY as N, getPositionFromPolar as O, get_store_value as P, rotateX as Q, cloneMatrix as R, SvelteComponent as S, appContext$1 as T, getTranslation as U, orthoNO as V, lookAt as W, linkProgram as X, validateProgram as Y, useProgram as Z, selectProgram as _, space as a, fromRotationTranslationScale as a0, createFlatShadedNormals as a1, toRadian$1 as a2, ARRAY_TYPE as a3, createVec3 as a4, lerp as a5, multiplyScalarVec3 as a6, normalizeNormals as a7, templateLiteralRenderer as a8, append_styles as a9, attr as aa, listen as ab, run_all as ac, create_slot as ad, toggle_class as ae, append as af, update_slot_base as ag, get_all_dirty_from_scope as ah, get_slot_changes as ai, text as aj, set_data as ak, createEventDispatcher as al, null_to_empty as am, hexNumToCSSStringColor as an, linearArrayToCSSHashColor as ao, cssStringColorToHexNum as ap, ensure_array_like as aq, empty as ar, group_outros as as, check_outros as at, destroy_each as au, cssStringColorToLinearArray as av, insert as b, create_component as c, transition_out as d, element as e, detach as f, destroy_component as g, component_subscribe as h, init as i, scene as j, materials as k, lights as l, mount_component as m, noop as n, onMount as o, camera as p, renderPasses as q, renderer as r, safe_not_equal as s, transition_in as t, transformMat4 as u, rotateZ as v, scale as w, translate as x, identity as y, createZeroMatrix as z };
